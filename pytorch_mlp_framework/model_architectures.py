...
class ConvolutionalProcessingBlock(nn.Module):
    ...
    def build_module(self):
        ...
        x = torch.zeros(self.input_shape)
        out = x
        self.b1 = nn.BatchNorm2d(self.num_filters)
...
class ConvolutionalDimensionalityReductionBlock(nn.Module):
    ...
    def build_module(self):
        ...
        out = self.layer_dict['conv_0'].forward(out)
        B = nn.BatchNorm2d(out.shape[1])
        out = F.leaky_relu(B(out))
    ...
    def forward(self, x):
        ...
        out = self.b1(out)
        out = F.max_pool2d(out, self.reduction_factor)
        x = F.max_pool2d(x,self.reduction_factor)
        out = self.layer_dict['conv_1'].forward(out)
        #out = self.b2(out)   
        out = F.leaky_relu(out+x)
        out = self.b2(out)
        out = self.m(out)
        return out
class BNormProcessingBlock(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
        super(BNormProcessingBlock, self).__init__()
...
class BNormDimensionalityReductionBlock(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation, reduction_factor):
        super(BNormDimensionalityReductionBlock, self).__init__()
